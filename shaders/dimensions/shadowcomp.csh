...
layout (local_size_x = 8, local_size_y = 8, local_size_z = 8) in;
    ...
    void PopulateSharedIndex(const in ivec3 imgCoordOffset, const in ivec3 workGroupOffset, const in uint i) {
        ...
        voxelSharedData[i] = GetVoxelBlock(pos);
            ...
            for (int l = 0; l < LPV_SHADOWS_LIGHT_COUNT_EFFECTIVE; l++) {
                float dist; ivec3 lpos; uint id;
                if (getLightData(texelFetch(texCloseLights, l, 0).r, dist, lpos, id)) {
                    if (length(vec3(pos) - vec3(lpos - 15 + int(LpvSize / 2))) < 1.5 && dist < 13.0) {
...
void main() {
        ...
        if (lightRange > 0.0) {
            ...
            #ifdef LPV_SHADOWS
                if (lightSharedData[getSharedIndex(ivec3(gl_LocalInvocationID) + 1)]) hsv.z *= LPV_SHADOWS_LIGHT_MULT;
            #endif
